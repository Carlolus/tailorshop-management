...
export class Step3GarmentsComponent implements OnInit {
    @Output() garmentsDataValid = new EventEmitter<{ garments: Garment[], measurements: Measurement[] } | null>();
        ...
        if (this.garmentsForm.valid && this.garmentsArray.length > 0) {
            ...
            formValue.garments.forEach((garmentData: any) => {
                ...
                const measurement: Measurement = {
                    garment_id: 0,
                    measures: garmentData.measurements
                };
            ...
            this.garmentsDataValid.emit({ garments, measurements });

        } else {
            this.garmentsDataValid.emit(null);
        }
    }
    ...
    getGarmentsData(): { garments: Garment[], measurements: Measurement[] } | null {
        if (!this.isFormValid()) {
            return null;
        }
